...
final class ExtensionManager: NSObject, ObservableObject,
    WKWebExtensionControllerDelegate, NSPopoverDelegate, WKNavigationDelegate, WKScriptMessageHandler, NSWindowDelegate
{
    ...
    // Keep weak references to options windows per extension id.
    // Some AppKit/WebKit flows can close/recreate windows, and we want to avoid
    // holding stale/dangling references that can crash on dictionary replacement.
    private final class WeakWindowBox {
        weak var window: NSWindow?
        init(_ window: NSWindow?) { self.window = window }
    }
    private var optionsWindows: [String: WeakWindowBox] = [:]
    // Separate map for action popups opened as standalone windows (e.g. uBlock Origin)
    private var popupWindows: [String: WeakWindowBox] = [:]
    ...
    private func setupExtensionController() {
        ...
        if !extensionDataStore.isPersistent {
            if #available(macOS 15.4, *) {
                let idString = extensionDataStore.identifier?.uuidString ?? "nil"
                Self.logger.error("Extension data store is NOT persistent (id=\(idString, privacy: .public)) – chrome.storage data may not survive restarts")
            } else {
                Self.logger.error("Extension data store is not persistent - this may cause storage issues")
            }
        } else if #available(macOS 15.4, *) {
            let idString = extensionDataStore.identifier?.uuidString ?? "nil"
            Self.logger.debug("Using persistent extension data store id=\(idString, privacy: .public)")
        }
        ...
        // Add custom feature message handlers (e.g. for downloads polyfill) to the shared config's userContentController
        // so that extension background workers and default popup configurations receive the handlers.
        for name in CustomFeatureRegistry.customHandlerNames() {
            sharedWebConfig.userContentController.removeScriptMessageHandler(forName: name)
            sharedWebConfig.userContentController.add(self, name: name)
        }
        ...
        -> WKWebsiteDataStore
    {
        ...
        if #available(macOS 15.4, *) {
            // Use a persistent store identified by the profile UUID for deterministic mapping
            store = WKWebsiteDataStore(forIdentifier: profileId)
            let idString = store.identifier?.uuidString ?? "nil"
            if !store.isPersistent {
                Self.logger.error("Extension data store for profile=\(profileId.uuidString, privacy: .public) is NOT persistent (id=\(idString, privacy: .public)) – chrome.storage data may not survive restarts")
            } else {
                Self.logger.debug("Created persistent extension data store for profile=\(profileId.uuidString, privacy: .public), id=\(idString, privacy: .public)")
            }
        } else {
            // Fallback for older OS versions (not officially supported, but safer than crashing)
            store = WKWebsiteDataStore.default()
            Self.logger.info("Using default extension data store (no per-profile store) for profile=\(profileId.uuidString, privacy: .public)")
        }
    ...
    func switchProfile(_ profileId: UUID) {
        // IMPORTANT:
        // WKWebExtensionController.configuration behaves like WKWebView.configuration (returns a COPY).
        // Mutating controller.configuration.defaultWebsiteDataStore after init may be a no-op, which
        // can cause MV3 chrome.storage.local to appear to "reset" or not persist correctly.
        //
        // To switch extension storage deterministically, rebuild the controller with the new store
        // set on the Configuration BEFORE creating the controller.
        Task { @MainActor in
            await rebuildExtensionController(for: profileId)
        }
    }

    private func rebuildExtensionController(for profileId: UUID) async {
        guard isExtensionSupportAvailable else { return }

        let enabledExtensions = installedExtensions.filter { $0.isEnabled }
        let enabledExtensionIDs = enabledExtensions.map { $0.id }

        // Unload currently loaded contexts from the old controller (best effort).
        if let oldController = extensionController {
            for extId in enabledExtensionIDs {
                if let ctx = extensionContexts[extId] {
                    try? oldController.unload(ctx)
                }
            }
        }

        // Update profile id BEFORE creating a new controller so setup picks the correct store.
        currentProfileId = profileId

        // Create a fresh controller bound to the new profile's website data store.
        // This does NOT clear any data; it just points the controller at the desired store.
        extensionController = nil
        setupExtensionController()

        guard let newController = extensionController else {
            Self.logger.error("Failed to rebuild WKWebExtensionController for profile=\(profileId.uuidString, privacy: .public)")
            return
        }

        // Recreate contexts for enabled extensions from disk so they're bound to the new controller/store.
        // Reusing existing WKWebExtensionContext instances can keep stale internal storage routing.
        for inst in enabledExtensions {
            let extId = inst.id
            let resourceURL = URL(fileURLWithPath: inst.packagePath)

            do {
                let webExtension = try await WKWebExtension(resourceBaseURL: resourceURL)
                let newContext = WKWebExtensionContext(for: webExtension)
                configureContextIdentity(newContext, extensionId: extId)

                // Grant permissions (matches our install/load behavior)
                for p in webExtension.requestedPermissions {
                    newContext.setPermissionStatus(.grantedExplicitly, for: p)
                }
                for p in webExtension.optionalPermissions {
                    newContext.setPermissionStatus(.grantedExplicitly, for: p)
                }
                for m in webExtension.allRequestedMatchPatterns {
                    newContext.setPermissionStatus(.grantedExplicitly, for: m)
                }
                for m in webExtension.optionalPermissionMatchPatterns {
                    newContext.setPermissionStatus(.grantedExplicitly, for: m)
                }

                newContext.isInspectable = true

                // Replace stored context
                extensionContexts[extId] = newContext

                // Ensure externally_connectable bridge is installed (safe to call multiple times).
                ...
                try newController.load(newContext)

                if webExtension.hasBackgroundContent {
                    newContext.loadBackgroundContent { [weak self] error in
                        if let error {
                            Self.logger.error("Background reload failed for '\(extId, privacy: .public)': \(error.localizedDescription, privacy: .public)")
                        } else {
                            Self.logger.info("Background restarted for '\(extId, privacy: .public)'")
                            self?.probeBackgroundHealth(for: newContext, name: inst.name)
                        }
                    }
                }
            } catch {
                Self.logger.error("Failed to recreate/reload extension '\(inst.name, privacy: .public)' (\(extId, privacy: .public)): \(error.localizedDescription, privacy: .public)")
                continue
            }
        }

        Self.logger.info("Rebuilt controller and reloaded enabled extensions for profile=\(profileId.uuidString, privacy: .public)")
    ...
    func windowWillClose(_ notification: Notification) {
        ...
        // Remove any stale options window entry pointing at this window
        let staleOptionKeys = optionsWindows
            .filter { $0.value.window === closedWindow }
            .map { $0.key }
        for key in staleOptionKeys {
            optionsWindows.removeValue(forKey: key)
        }

        // Remove any stale popup window entry pointing at this window
        let stalePopupKeys = popupWindows
            .filter { $0.value.window === closedWindow }
            .map { $0.key }
        for key in stalePopupKeys {
    ...
    private func patchManifestForWebKit(at manifestURL: URL) {
        ...
        // --- Inject nook_polyfill.js for chrome.downloads in all content scripts ---
        ...
        chrome.downloads.download = chrome.downloads.download || function(options, callback) {
            ...
            } catch(e) { console.error("Download polyfill error", e); }
        };
        browser.downloads = chrome.downloads;
        
        // Polyfill missing webNavigation events that cause UBlock Origin popups to throw errors and go blank
        ...
        """
        let polyfillURL = dir.appendingPathComponent("nook_polyfill.js")
        if (try? String(contentsOf: polyfillURL)) != polyfillJS {
            try? polyfillJS.write(to: polyfillURL, atomically: true, encoding: .utf8)
        }
        
        var contentScripts = manifest["content_scripts"] as? [[String: Any]] ?? []
        let hasPolyfill = contentScripts.contains { dict in
            if let js = dict["js"] as? [String] {
                return js.contains("nook_polyfill.js")
            }
            return false
        }
        if !hasPolyfill {
            let polyfillEntry: [String: Any] = [
                "matches": ["<all_urls>"],
                "js": ["nook_polyfill.js"],
                "run_at": "document_start",
                "all_frames": true,
            ...
            // We insert it at the very beginning of the content scripts so it's guaranteed
            // to load before any of the extension's own scripts.
            contentScripts.insert(polyfillEntry, at: 0)
            manifest["content_scripts"] = contentScripts
            changed = true
            Self.logger.info("Registered nook_polyfill.js in manifest")
        }

        // Write the patch files out.
        let patchJS = """
        (function() {
            if (typeof chrome === 'undefined') { window.chrome = {}; }
            if (typeof browser === 'undefined') { window.browser = window.chrome; }
            ...
            function fixTabsQuery(obj) {
                if (!obj || !obj.tabs || !obj.tabs.query) return;
                if (obj.tabs._nookPatchedQuery) return;
                var origQuery = obj.tabs.query;
                obj.tabs.query = function(queryInfo, callback) {
                    var fallbackUrl = window.location.href; // At least it's a URL
                    var ret = origQuery.call(obj.tabs, queryInfo, function(tabs) {
                        if (tabs) {
                ...
                obj.tabs._nookPatchedQuery = true;
            }

            fixTabsQuery(window.chrome);
            fixTabsQuery(window.browser);
        ...
        """
        let patchURL = dir.appendingPathComponent("nook_patch.js")
        if (try? String(contentsOf: patchURL)) != patchJS {
            try? patchJS.write(to: patchURL, atomically: true, encoding: .utf8)
        }
        
        let hasPatch = contentScripts.contains { dict in
            if let js = dict["js"] as? [String] {
                return js.contains("nook_patch.js")
            }
            return false
        }
        if !hasPatch {
            let patchEntry: [String: Any] = [
                "matches": ["<all_urls>"],
                "js": ["nook_patch.js"],
                "run_at": "document_start",
                "all_frames": true,
            ...
            contentScripts.insert(patchEntry, at: 0)
            manifest["content_scripts"] = contentScripts
            changed = true
            Self.logger.info("Registered nook_patch.js in manifest")
        }
        ...
        // --- VidIQ Vision: ensure YouTube Studio is included for content scripts and host permissions ---
        // VidIQ (Chrome extension) often lists www.youtube.com / *.youtube.com but not studio.youtube.com.
        // Adding studio.youtube.com ensures the extension runs on YouTube Studio as well.
        let extName = (manifest["name"] as? String) ?? ""
        let isVidIQ = extName.range(of: "vidiq", options: .caseInsensitive) != nil
        if isVidIQ {
            let studioPattern = "*://studio.youtube.com/*"
            func patternMatchesYouTube(_ p: String) -> Bool { p.contains("youtube.com") }
            func patternsIncludeStudio(_ ps: [String]) -> Bool { ps.contains { $0.contains("studio.youtube.com") } }

            if var hostPerms = manifest["host_permissions"] as? [String],
               hostPerms.contains(where: patternMatchesYouTube),
               !patternsIncludeStudio(hostPerms) {
                hostPerms.append(studioPattern)
                manifest["host_permissions"] = hostPerms
                changed = true
                Self.logger.info("patchManifestForWebKit: added studio.youtube.com to host_permissions for VidIQ")
            }
            // MV2 extensions use "permissions" for host patterns
            if var perms = manifest["permissions"] as? [String],
               perms.contains(where: patternMatchesYouTube),
               !patternsIncludeStudio(perms) {
                perms.append(studioPattern)
                manifest["permissions"] = perms
                changed = true
                Self.logger.info("patchManifestForWebKit: added studio.youtube.com to permissions for VidIQ")
            }

            if var contentScripts = manifest["content_scripts"] as? [[String: Any]] {
                var contentScriptsChanged = false
                for i in contentScripts.indices {
                    guard var matches = contentScripts[i]["matches"] as? [String],
                          matches.contains(where: patternMatchesYouTube),
                          !patternsIncludeStudio(matches) else { continue }
                    if (contentScripts[i]["js"] as? [String])?.contains("nook_bridge.js") == true { continue }
                    matches.append(studioPattern)
                    contentScripts[i]["matches"] = matches
                    contentScriptsChanged = true
                    changed = true
                }
                manifest["content_scripts"] = contentScripts
                if contentScriptsChanged { Self.logger.info("patchManifestForWebKit: added studio.youtube.com to content_scripts matches for VidIQ") }
            }
        }
        ...
        async throws
    {
        ...
        // unzip returns 1 for warnings (like CRX header), 0 for success
    ...
    private func resolveSafariExtensionResources(at bundleURL: URL) throws -> URL {
        ...
        if ext == "appex" {
            // First check for manifest.json directly in Contents/Resources
            ...
            // Some extensions nest resources deeper — check for a Resources subdirectory
            if let items = try? FileManager.default.contentsOfDirectory(at: resourcesDir, includingPropertiesForKeys: nil) {
                for item in items {
                    let nestedManifest = item.appendingPathComponent("manifest.json")
                    if FileManager.default.fileExists(atPath: nestedManifest.path) {
                        Self.logger.info("Found Safari extension resources in nested dir: \(item.path, privacy: .public)")
                        return item
                    }
                }
            }
        ...
        if ext == "app" {
            ...
            // Prefer the one with NSExtensionPointIdentifier == com.apple.Safari.web-extension
            ...
            if let items = try? FileManager.default.contentsOfDirectory(
                ...
                for item in items where item.pathExtension.lowercased() == "appex" {
                    // Check if this is a web extension (not a content blocker)
                    ...
                    if let plistData = try? Data(contentsOf: infoPlist),
                       ...
                       pointId == "com.apple.Safari.web-extension" {
                        ...
                        let manifest = resourcesDir.appendingPathComponent("manifest.json")
                        if FileManager.default.fileExists(atPath: manifest.path) {
                            Self.logger.info("Found Safari web extension in \(item.lastPathComponent): \(resourcesDir.path, privacy: .public)")
                            return resourcesDir
                        }
                    }
                }
                // Fallback: any appex with a manifest.json
                for item in items where item.pathExtension.lowercased() == "appex" {
                    let resourcesDir = item.appendingPathComponent("Contents/Resources")
                    let manifest = resourcesDir.appendingPathComponent("manifest.json")
                    if FileManager.default.fileExists(atPath: manifest.path) {
                        Self.logger.info("Found extension in \(item.lastPathComponent) (fallback): \(resourcesDir.path, privacy: .public)")
                        return resourcesDir
                    }
                }
            }
    ...
    /// Update whether an extension is pinned to the toolbar (shown next to the URL bar).
    func setExtensionPinned(_ extensionId: String, pinned: Bool) {
        do {
            ...
            if let entity = entities.first {
                entity.isPinnedToToolbar = pinned
                try self.context.save()
            ...
            Self.logger.error("Failed to update extension pinned state: \(error.localizedDescription, privacy: .public)")
        }
    }
    ...
    struct SafariExtensionInfo: Identifiable {
        ...
        let isWebAppVariant: Bool // true if this is a "for Web Apps" variant (better WKWebView compat)
    }
        ...
        return await withCheckedContinuation { continuation in
            DispatchQueue.global(qos: .userInitiated).async {
                ...
                for searchDir in searchDirs {
                    ...
                    for appURL in apps where appURL.pathExtension == "app" {
                        ...
                        for pluginURL in plugins where pluginURL.pathExtension == "appex" {
                            ...
                            // Read extension name from manifest, resolving __MSG_ localizations
                            ...
                            if let data = try? Data(contentsOf: manifestPath),
                               ...
                               let name = manifest["name"] as? String {
                                if name.hasPrefix("__MSG_") {
                                    let msgKey = name.dropFirst(6).dropLast(2)
                                    let resolved = Self.resolveLocalizedMessage(String(msgKey), in: resourcesDir)
                            ...
                            let isWebApp = appName.lowercased().contains("web app")
                ...
                let sorted = results.sorted { a, b in
                    if a.isWebAppVariant != b.isWebAppVariant { return a.isWebAppVariant }
    ...
    /// Resolve a __MSG_key__ reference from _locales/en/messages.json (or en_US, default_locale).
    private static func resolveLocalizedMessage(_ key: String, in resourcesDir: URL) -> String? {
        let locales = ["en", "en_US", "en_GB"]
        for locale in locales {
            let messagesURL = resourcesDir.appendingPathComponent("_locales/\(locale)/messages.json")
    ...
    private func loadInstalledExtensions() {
        ...
        Task { @MainActor in
            ...
            for (entity, webExtension) in parsed {
                ...
                if webExtension.hasBackgroundContent {
                    // Attempt to inject polyfills as early as possible into the background webview
                    // (some extensions run inline bootstrap code very early).
                    // #region agent log
                    let preBgWV = (extensionContext as AnyObject).value(forKey: "_backgroundWebView") as? WKWebView
                    Self.debugLog("background.preInject", data: [
                        "hypothesisId": "H2",
                        "name": entity.name,
                        "hasWebView": preBgWV != nil,
                        "url": preBgWV?.url?.absoluteString ?? "nil"
                    ])
                    // #endregion
                    if let preBgWV {
                        let polyfill = CustomFeatureRegistry.downloadsPolyfillScript()
                        preBgWV.configuration.userContentController.addUserScript(polyfill)
                    }
                    ...
                    extensionContext.loadBackgroundContent(completionHandler: { [weak self] error in
                        if let error {
                            ...
                            // Inject custom polyfills into the background webview (e.g. chrome.downloads)
                            if let bgWV = (extensionContext as AnyObject).value(forKey: "_backgroundWebView") as? WKWebView {
                                let polyfill = CustomFeatureRegistry.downloadsPolyfillScript()
                                bgWV.configuration.userContentController.addUserScript(polyfill)
                                bgWV.evaluateJavaScript(polyfill.source)
                            }
    ...
    func webExtensionController(
        _ controller: WKWebExtensionController,
        tabFor webView: WKWebView
    ) -> (any WKWebExtensionTab)? {
        guard let browserManager = browserManagerRef else { return nil }
        
        // Search all tabs across all spaces and pinned states
        for tab in browserManager.tabManager.allTabs() {
            // Check the primary webview and all window-specific clone webviews
            if tab.existingWebView === webView || (browserManager.webViewCoordinator?.getAllWebViews(for: tab.id) ?? []).contains(where: { $0 === webView }) {
                return stableAdapter(for: tab)
            }
        }
    ...
    func webExtensionController(
        ...
        // #region agent log
        Self.debugLog("presentActionPopup.start", data: [
            "hypothesisId": "H1",
            "extensionName": extName,
            "hasPopupPopover": action.popupPopover != nil,
            "hasPopupWebView": action.popupWebView != nil
        ])
        // #endregion
        ...
        if let webView = action.popupWebView {
            ...
            // Remove previous user scripts to prevent accumulation and infinite loops
            webView.configuration.userContentController.removeAllUserScripts()
            
            // Add custom message handlers to the popup
            for name in CustomFeatureRegistry.customHandlerNames() {
                // Remove first to avoid duplicate handler exceptions
                webView.configuration.userContentController.removeScriptMessageHandler(forName: name)
                webView.configuration.userContentController.add(self, name: name)
            }

            // Ensure extension storage shim is available in popups as well.
            webView.configuration.userContentController.addUserScript(
                CustomFeatureRegistry.extensionStorageShimScript()
            )
            
            // #region agent log
            Self.debugLog("popup.webViewConfig", data: [
                "hypothesisId": "H1",
        ...
        // Special-case uBlock Origin: show popup in its own window instead of a popover
        if extName == "uBlock Origin", let popupWebView = action.popupWebView {
            let window = NSWindow(
            ...
            if let extId = extensionContexts.first(where: { $0.value === extensionContext })?.key {
                popupWindows[extId] = WeakWindowBox(window)
            }
        ...
        DispatchQueue.main.async {
            ...
            // #region agent log
            if let webView = action.popupWebView {
                let activeUrl = self.browserManagerRef?.currentTabForActiveWindow()?.url.absoluteString ?? "about:blank"
                let js = """
                window.onerror = function(msg, url, line, col, error) {
                    var out = msg + " at " + url + ":" + line;
                    if (error && error.stack) { out += "\\nStack: " + error.stack; }
                    if (window.webkit && window.webkit.messageHandlers.nookPopupError) {
                        window.webkit.messageHandlers.nookPopupError.postMessage(out);
                    }
                };
                window.addEventListener('unhandledrejection', function(event) {
                    var msg = "Unhandled promise rejection: " + event.reason;
                    if (event.reason && event.reason.stack) { msg += "\\nStack: " + event.reason.stack; }
                    if (window.webkit && window.webkit.messageHandlers.nookPopupError) {
                        window.webkit.messageHandlers.nookPopupError.postMessage(msg);
                    }
                });

                // Ensure localStorage is never null inside extension popups (vidIQ expects a real object).
                try {
                ...
                // Popup environment debug logging to a localhost HTTP endpoint
                // has been removed to avoid repeated failing connections when
                // the debug server is not running.
                
                // --- FIX FOR UBLOCK ORIGIN ---
                // uBlock's UI tries to read chrome.webNavigation properties synchronously during setup
                // but the WKWebExtension object might be initialized asynchronously or missing certain event listeners.
                function fixExtensionAPI(obj) {
                    if (!obj) return;
                    if (!obj.webNavigation) obj.webNavigation = {};
                    if (!obj.webNavigation.onHistoryStateUpdated) obj.webNavigation.onHistoryStateUpdated = { addListener: function() {}, removeListener: function() {}, hasListener: function() { return false; } };
                    if (!obj.webNavigation.onReferenceFragmentUpdated) obj.webNavigation.onReferenceFragmentUpdated = { addListener: function() {}, removeListener: function() {}, hasListener: function() { return false; } };
                    
                    if (obj.runtime && obj.runtime.onConnect && !obj.runtime.onConnect.hasListeners) {
                        var origOnConnect = obj.runtime.onConnect;
                        var origAddListener = origOnConnect.addListener;
                        try {
                            obj.runtime.onConnect.addListener = function(listener) {
                                return origAddListener.call(origOnConnect, function(port) {
                                    if (!port.sender) {
                                        port.sender = { id: obj.runtime.id, url: "webkit-extension://fallback", origin: "webkit-extension://fallback" };
                                    }
                                    return listener(port);
                                });
                            };
                            obj.runtime.onConnect.hasListeners = function() { return false; };
                        } catch(e) {}
                    }

                    if (obj.runtime && obj.runtime.onMessage && !obj.runtime.onMessage.hasListeners) {
                        try {
                            obj.runtime.onMessage.hasListeners = function() { return false; };
                        } catch(e) {}
                    }
                    
                    // Polyfill tabs.query to inject fallback URL if WebKit stripped it due to permissions (e.g. nook:// or about:blank)
                    if (obj.tabs && obj.tabs.query) {
                        var origQuery = obj.tabs.query;
                        obj.tabs.query = function(queryInfo, callback) {
                            var ret = origQuery.call(obj.tabs, queryInfo, function(tabs) {
                                if (tabs) {
                ...
                if (typeof chrome !== 'undefined') fixExtensionAPI(chrome);
                if (typeof browser !== 'undefined') fixExtensionAPI(browser);
                """
                let script = WKUserScript(source: js, injectionTime: .atDocumentStart, forMainFrameOnly: false)
                webView.configuration.userContentController.addUserScript(script)
                
                // Ensure webview fills its container if it isn't already
                if let superview = webView.superview {
                    webView.translatesAutoresizingMaskIntoConstraints = true
                    webView.autoresizingMask = [.width, .height]
                    if webView.frame.width == 0 {
                ...
                // REMOVED manual load call because WebKit automatically navigates the popup webview
                // and calling it manually might destroy the WKWebExtension context APIs.
                Self.debugLog("popup.webViewStatus", data: [
                    "hypothesisId": "H1",
                    "url": webView.url?.absoluteString ?? "nil",
                    "isLoading": webView.isLoading,
                    "estimatedProgress": webView.estimatedProgress,
                    "hasOnlySecureContent": webView.hasOnlySecureContent,
                    "popoverSize": ["width": popover.contentSize.width, "height": popover.contentSize.height],
                    "webViewFrame": ["width": webView.frame.size.width, "height": webView.frame.size.height]
                ])
                
                // Workaround: if the popover size is tiny, give it a default size
                if popover.contentSize.width < 10 || popover.contentSize.height < 10 {
                    popover.contentSize = NSSize(width: 400, height: 500)
                    if let vc = popover.contentViewController {
                        vc.view.setFrameSize(NSSize(width: 400, height: 500))
                        vc.preferredContentSize = NSSize(width: 400, height: 500)
                    }
                    webView.setFrameSize(NSSize(width: 400, height: 500))
                }
            }
            // #endregion
                ...
                var anchors = self.actionAnchors[extId]
            {
                    ...
                    view.window != nil  // Double-check view is still in window
                {
                    ...
                    // #region agent log
                    Self.debugLog("popover.shown", data: ["hypothesisId": "H1", "anchor": "valid window"])
                    // #endregion
                    completionHandler(nil)
                    return
                }
                   ...
                   let view = validAnchor.view
                {
                    ...
                    // #region agent log
                    Self.debugLog("popover.shown", data: ["hypothesisId": "H1", "anchor": "first available"])
                    // #endregion
                    completionHandler(nil)
                    return
                }
            ...
            if let window = targetWindow, let contentView = window.contentView {
                ...
                // #region agent log
                Self.debugLog("popover.shown", data: ["hypothesisId": "H1", "anchor": "fallback"])
                // #endregion
                completionHandler(nil)
                return
            }
    ...
    func userContentController(
        ...
        if CustomFeatureRegistry.handleMessage(name: message.name, message: message) {
            return
        }
        if message.name == "nookExtensionStorage" {
            handleExtensionStorageMessage(message)
            return
        }
    }

    // MARK: - Host-backed extension storage shim

    private func handleExtensionStorageMessage(_ message: WKScriptMessage) {
        guard let body = message.body as? [String: Any] else { return }
        guard let requestId = body["requestId"] as? String, !requestId.isEmpty else {
            return
        }
        guard let op = body["op"] as? String, !op.isEmpty else {
            respondToStorage(requestId: requestId, webView: message.frameInfo.webView ?? message.webView, error: "Missing op")
            return
        }

        // Extension ID comes from runtime.id in JS; if absent, fall back to mapping from URL host.
        let extensionId = (body["extensionId"] as? String)
            ?? extensionIdFromWebViewURL(message.frameInfo.webView?.url ?? message.webView?.url)
            ?? "unknown"

        // If we don't have a profile yet, use the controller identifier (still persistent) as a fallback namespace.
        let profileNamespace = currentProfileId?.uuidString
            ?? extensionController?.configuration.identifier?.uuidString
            ?? "default"

        let payload = body["payload"] as? [String: Any]

        do {
            Self.logger.debug("[ExtStorage] op=\(op, privacy: .public) profile=\(profileNamespace, privacy: .public) ext=\(extensionId, privacy: .public)")
            let result = try performStorageOp(
                op: op,
                profileNamespace: profileNamespace,
                extensionId: extensionId,
                payload: payload
            )
            respondToStorage(requestId: requestId, webView: message.frameInfo.webView ?? message.webView, result: result)
        } catch {
            respondToStorage(requestId: requestId, webView: message.frameInfo.webView ?? message.webView, error: error.localizedDescription)
        }
    }

    private func respondToStorage(requestId: String, webView: WKWebView?, result: Any) {
        guard let webView else { return }
        guard JSONSerialization.isValidJSONObject(result) else {
            respondToStorage(requestId: requestId, webView: webView, error: "Invalid JSON result")
            return
        }
        guard let data = try? JSONSerialization.data(withJSONObject: result),
              let json = String(data: data, encoding: .utf8)
        else {
            respondToStorage(requestId: requestId, webView: webView, error: "Failed to encode result")
            return
        }

        let safeRequestId = requestId.replacingOccurrences(of: "\\", with: "\\\\").replacingOccurrences(of: "'", with: "\\'")
        let js = "try { window.__nookStorageResolve('\(safeRequestId)', \(json)); } catch (e) {}"
        webView.evaluateJavaScript(js)
    }

    private func respondToStorage(requestId: String, webView: WKWebView?, error: String) {
        guard let webView else { return }
        let safeRequestId = requestId.replacingOccurrences(of: "\\", with: "\\\\").replacingOccurrences(of: "'", with: "\\'")
        let safeError = error.replacingOccurrences(of: "\\", with: "\\\\").replacingOccurrences(of: "'", with: "\\'")
        let js = "try { window.__nookStorageReject('\(safeRequestId)', '\(safeError)'); } catch (e) {}"
        webView.evaluateJavaScript(js)
    }

    private func extensionIdFromWebViewURL(_ url: URL?) -> String? {
        guard let url else { return nil }
        guard let host = url.host, !host.isEmpty else { return nil }
        // Match our deterministic baseURL host: "ext-" + hex(extensionId bytes)
        for (extId, ctx) in extensionContexts {
            if let ctxHost = ctx.baseURL.host, ctxHost == host {
                return extId
            }
        }
        return nil
    ...
    private func storageBaseDirectory() -> URL {
    ...
    private func storageFileURL(profileNamespace: String, extensionId: String) -> URL {
        storageBaseDirectory()
            .appendingPathComponent(profileNamespace, isDirectory: true)
            .appendingPathComponent("\(extensionId).json", isDirectory: false)
    }

    private func loadStorage(profileNamespace: String, extensionId: String) throws -> [String: Any] {
        let url = storageFileURL(profileNamespace: profileNamespace, extensionId: extensionId)
    ...
    private func saveStorage(_ dict: [String: Any], profileNamespace: String, extensionId: String) throws {
        let url = storageFileURL(profileNamespace: profileNamespace, extensionId: extensionId)
    ...
    private func performStorageOp(
        ...
        switch op {
        case "get":
            let store = try loadStorage(profileNamespace: profileNamespace, extensionId: extensionId)
            let keys = payload?["keys"]

            // keys: null|undefined => all; string => one; [strings] => many; {defaults} => many with defaults
            if keys is NSNull || keys == nil {
                return store
            }
            if let k = keys as? String {
                return [k: store[k] ?? NSNull()]
            }
            if let arr = keys as? [String] {
                var out: [String: Any] = [:]
                for k in arr {
                    out[k] = store[k] ?? NSNull()
                }
                return out
            }
        ...
        case "set":
            var store = try loadStorage(profileNamespace: profileNamespace, extensionId: extensionId)
            let items = (payload?["items"] as? [String: Any]) ?? [:]
            for (k, v) in items {
                // Only persist JSON-serializable values; drop others.
                if JSONSerialization.isValidJSONObject([k: v]) || v is String || v is NSNumber || v is NSNull || v is [Any] || v is [String: Any] {
                    store[k] = v
                } else {
                    store[k] = String(describing: v)
                }
            }
            try saveStorage(store, profileNamespace: profileNamespace, extensionId: extensionId)
            return ["ok": true]
        ...
        case "clear":
            try saveStorage([:], profileNamespace: profileNamespace, extensionId: extensionId)
            return ["ok": true]

        default:
            throw NSError(domain: "ExtensionStorage", code: 1, userInfo: [NSLocalizedDescriptionKey: "Unknown op: \(op)"])
        }
    }

    // MARK: - WKNavigationDelegate (popup diagnostics)
    // #region agent log
    private static func debugLog(_ message: String, data: [String: Any]) {
        // Intentionally left as a no-op in normal builds.
        // The original implementation POSTed diagnostics to a localhost HTTP
        // endpoint on every popup navigation event, which created a lot of
        // failing connections (and log noise) when that debug server was not running.
        _ = (message, data)
    }
    // #endregion
    ...
    func webView(
        ...
        // #region agent log
        Self.debugLog("popup.didStart", data: [
            "hypothesisId": "H1",
            "url": urlString
        ])
        // #endregion
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
        ...
        // #region agent log
        Self.debugLog("popup.didFinish", data: [
            "hypothesisId": "H1",
            "url": urlString
        ])
        // #endregion
    ...
    func webView(
        ...
        // #region agent log
        Self.debugLog("popup.didFail", data: [
            "hypothesisId": "H1",
            "url": urlString,
    ...
    func webView(
        ...
        // #region agent log
        Self.debugLog("popup.didFailProvisional", data: [
            "hypothesisId": "H1",
            "url": urlString,
            "error": error.localizedDescription
        ])
    ...
    func webViewWebContentProcessDidTerminate(_ webView: WKWebView) {
        ...
        // #region agent log
        Self.debugLog("popup.processTerminated", data: ["hypothesisId": "H1"])
        // #endregion
    }
    ...
    func webExtensionController(
        ...
        // #region agent log
        Self.debugLog("native.sendMessage", data: [
            "hypothesisId": "NM1",
            "applicationId": applicationId,
            "extension": extensionContext.webExtension.displayName ?? "?",
            "runId": "pre-fix"
        ])
        // #endregion
    ...
    func webExtensionController(
        ...
        // #region agent log
        Self.debugLog("native.connectPort", data: [
            "hypothesisId": "NM1",
            "applicationId": applicationId,
            "extension": extensionContext.webExtension.displayName ?? "?",
            "runId": "pre-fix"
    ...
    func webExtensionController(
        ...
        // Ensure our host-backed extension storage handler is available in this options WebView.
        // Options pages use a separate userContentController from the shared config.
        for name in CustomFeatureRegistry.customHandlerNames() {
            webView.configuration.userContentController.removeScriptMessageHandler(forName: name)
            webView.configuration.userContentController.add(self, name: name)
        }

        // Ensure the storage shim script itself is injected into the options page.
        // (Adding the message handler alone is not enough.)
        webView.configuration.userContentController.addUserScript(
            CustomFeatureRegistry.extensionStorageShimScript()
        )
        ...
        // Try to reuse an existing options window for this extension if it exists.
        var existingWindow: NSWindow?
        var extIdForOptions: String?
        if let extId = extensionContexts.first(where: { $0.value === extensionContext })?.key {
            extIdForOptions = extId
            existingWindow = optionsWindows[extId]?.window
        }
        ...
        // If we had an entry but it has since been deallocated, clear it.
        if let extIdForOptions, optionsWindows[extIdForOptions]?.window == nil {
            optionsWindows.removeValue(forKey: extIdForOptions)
        }
        ...
        if let extId = extIdForOptions {
            optionsWindows[extId] = WeakWindowBox(window)
        }
        ...
        for delay in [3.0, 8.0] {
            DispatchQueue.main.asyncAfter(deadline: .now() + delay) {
                guard let bgWV = (context as AnyObject).value(forKey: "_backgroundWebView") as? WKWebView else {
                    ...
                    // #region agent log
                    Self.debugLog("background.probeNoWebView", data: ["hypothesisId": "H3", "name": name, "delay": delay])
                    // #endregion
                    return
                }
                ...
                """) { result, error in
                    if let json = result as? String {
                        ...
                        // #region agent log
                        Self.debugLog("background.probeSuccess", data: ["hypothesisId": "H3", "name": name, "delay": delay, "result": json])
                        // #endregion
                    } else if let error = error {
                        NSLog("[EXT-HEALTH] [\(name)] +\(Int(delay))s probe FAILED: \(error.localizedDescription)")
                        // #region agent log
                        Self.debugLog("background.probeError", data: ["hypothesisId": "H3", "name": name, "delay": delay, "error": error.localizedDescription])
                        // #endregion
                    }
                }
            }
    ...
    func diagnoseExtensionState(for webView: WKWebView, url: URL) {
        ...
        // #region agent log
        Self.debugLog("diagnoseExtensionState", data: [
            "hypothesisId": "H1",
            "host": host,
            "ctxCount": ctxCount,
            "hasController": configCtrl != nil,
            "isSameController": sameCtrl
        ])
        // #endregion
        ...
        for (extId, ctx) in extensionContexts {
            ...
            // #region agent log
            Self.debugLog("diagnoseExtensionState.ext", data: [
                "hypothesisId": "H1",
                "name": name,
