...
enum WebContextMenuItem {
    ...
    private static func content(for payload: WebContextMenuPayload) -> [WebContextMenuItem] {
        switch payload {
        case .page:
            return [.pageBack, .pageForward, .pageReload, .separator, .pageCopyAddress, .separator, .readerMode, .saveToZotero, .separator, .openInNativePlayer, .blockElement, .separator, .systemInspect]
        case .textSelection:
    ...
    private var title: String? {
        ...
        case .openInNativePlayer: return "Open Media in Native Player"
        case .blockElement: return "Block Element..."
        case .readerMode: return "Enter Reader Mode"
        case .saveToZotero: return "Save to Zotero"
    ...
    private func performAction(on webView: FocusableWKWebView, payload: WebContextMenuPayload) {
        ...
        case .openInNativePlayer:
            CanopyNativePlayer.shared.openCurrentMedia(from: webView)
        case .blockElement:
            CanopyElementPicker.shared.activate(in: webView)
        case .readerMode:
            CanopyReaderMode.shared.activate(in: webView)
        case .saveToZotero:
            ZoteroConnector.shared.saveCurrentPage(from: webView)
        default:
            break
        }
    }
