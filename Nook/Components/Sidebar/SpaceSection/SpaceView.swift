...
struct SpaceView: View {
    ...
    private func pinnedTabContextMenu(_ tab: Tab) -> some View {
        VStack {
            ...
            } label: { Label("Edit Pinned URL", systemImage: "pencil") }
    ...
    private func regularTabContextMenu(_ tab: Tab) -> some View {
        VStack {
            ...
            Divider()
            Button { browserManager.tabManager.pinTabToSpace(tab, spaceId: space.id) } label: { Label("Move to Pinned Tabs", systemImage: "pin") }
            Button { onPinTab(tab) } label: { Label("Pin to Favorites", systemImage: "star.fill") }
            Button { onCloseTab(tab) } label: { Label("Close tab", systemImage: "xmark") }
        }
    }
