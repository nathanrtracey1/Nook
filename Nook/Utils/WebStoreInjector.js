...
(function () {
  ...
  function replaceInstallButton() {
    ...
    const selectors = [
      'div[role="button"][class*="gY-y-s"]', // New Chrome Web Store main button
      'div[role="button"][jsaction*="click"] span', // New Chrome Web Store alternative
      'button[jsname]', // New Chrome Web Store generic button
    ...
    for (const selector of selectors) {
      ...
      for (const button of buttons) {
        ...
        if (
          ...
          text.includes("available on chrome") ||
          text.includes("download chrome") ||
          ariaLabel.includes("add to chrome") ||
          ariaLabel.includes("remove from chrome") ||
          ariaLabel.includes("available on chrome") ||
          ariaLabel.includes("download chrome") ||
          text === "unavailable" ||
          button.hasAttribute("disabled")
        ) {
          // Additional check for generic buttons like div[role="button"]: Ensure they're not just random UI buttons
          if (selector === 'div[role="button"]') {
            // Only replace if it contains a recognizable install label or has specific classes/attributes
            // The new web store often uses aria-label="Add to Chrome" which is caught by ariaLabel check above
            // If it matches by text only, and the text is broad, it might be safe, but let's let it pass
            // since we checked the specific text/ariaLabel strings above
          }
