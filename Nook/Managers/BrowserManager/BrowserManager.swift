...
class BrowserManager: ObservableObject {
    ...
    private func enforceExclusiveAudio(
        ...
        _ = desiredMuteState ?? tab.isAudioMuted
    ...
    init() {
        ...
        // Observe thermal state changes as a proxy for memory/CPU pressure and
        // aggressively unload inactive tabs when the system is under stress.
        memoryPressureObserver = NotificationCenter.default.addObserver(
            ...
            self.tabManager.suspendInactiveTabs()
        }
        self.trackingProtectionManager.attach(browserManager: self)
        self.contentBlockerManager.attach(browserManager: self)

        // Canopy Ad Blocker: pre-compile bundled rule lists on launch
        CanopyAdBlockManager.shared.setEnabled(true)
        Task {
            await CanopyAdBlockManager.shared.compileRuleLists()
            await CanopyAdBlockManager.shared.updateListsIfNeeded()
            await WBlockBridge.shared.loadIfAvailable()
        }
        // Note: tracking protection and content blocker will be configured after settingsManager injection
        ...
        NotificationCenter.default.addObserver(
            forName: .contentBlockingEnabledChanged,
            object: nil,
            queue: .main
        ) { [weak self] note in
            guard let enabled = note.userInfo?["enabled"] as? Bool else { return }
        ...
        NotificationCenter.default.addObserver(
            forName: .contentBlockerAllowlistChanged,
            object: nil,
            queue: .main
        ) { [weak self] note in
            guard let hosts = note.userInfo?["hosts"] as? [String] else { return }
            Task { @MainActor [weak self] in
                self?.contentBlockerManager.setAllowedDomains(hosts)
            }
        }

        NotificationCenter.default.addObserver(
            forName: .contentBlockerListsChanged,
            object: nil,
            queue: .main
        ) { [weak self] note in
            ...
            let set = Set(ids)
            Task { @MainActor [weak self] in
                self?.contentBlockerManager.setEnabledSourceIDs(set.isEmpty ? nil : set)
            }
        }

        NotificationCenter.default.addObserver(
            forName: .contentBlockerAggressiveModeChanged,
            object: nil,
            queue: .main
        ) { [weak self] note in
            guard let enabled = note.userInfo?["enabled"] as? Bool else { return }
            Task { @MainActor [weak self] in
                self?.contentBlockerManager.setAggressiveMode(enabled)
            }
        }
    ...
    func maybeShowOAuthAssist(for url: URL, in tab: Tab) {
        ...
        // Don't show for known sign-in providers; we already skip tracking there so sign-in and password manager work
        guard !OAuthDetector.matchesKnownProvider(host: host) else { return }
    ...
    func selectTab(_ tab: Tab, in windowState: BrowserWindowState) {
        ...
        // Load the tab's webview if it was unloaded (does nothing if already loaded)
        ...
        // Don't refresh compositor here: windowState.currentTabId change already triggers SwiftUI
        // to re-render; compositor updates visibility in updateNSView. Avoiding refreshCompositor()
        // prevents unnecessary compositor runs that can cause visible reloads.
...
/// DEPRECATED: Use WebViewCoordinator.getWebView() directly via environment
    ...
    /// Show history in the sidebar menu (History tab), similar to Safari's history view.
    func showHistory() {
        guard let windowState = windowRegistry?.activeWindow else { return }
        
        withAnimation(.easeInOut(duration: 0.2)) {
            windowState.savedSpaceIdWhenMenuOpened = windowState.currentSpaceId
            ...
            // Use a wider sidebar width for the history view
