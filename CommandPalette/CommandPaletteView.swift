...
struct CommandPaletteView: View {
    ...
    /// Active window width, kept in sync via notifications to avoid recomputing
    /// NSApplication.shared.keyWindow on every keystroke.
    @State private var currentWindowWidth: CGFloat = 0
    ...
    private var isWindowTooNarrow: Bool {
        ...
        return currentWindowWidth > 0 && currentWindowWidth <= requiredWidth
    }
    
    /// Calculate the correct command palette width
    ...
    var body: some View {
        ...
        .onAppear {
            if let width = NSApplication.shared.keyWindow?.frame.width {
                currentWindowWidth = width
            }
        }
        .onReceive(NotificationCenter.default.publisher(for: NSWindow.didResizeNotification)) { notification in
            guard let window = notification.object as? NSWindow,
                  window == NSApplication.shared.keyWindow else { return }
            currentWindowWidth = window.frame.width
        }
    }
