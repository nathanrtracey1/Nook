    ...
    func createBoostUserScripts(for config: BoostConfig, domain: String) -> [WKUserScript] {
        ...
        let combinedScript = """
            ...
            (function() {
                ...
                \(fontPreloadScript)
                // Attach styles as early as possible; document.head may not exist yet.
                (function ensureHeadOrDocumentElement(style) {
                    var target = document.head || document.documentElement;
                    if (!target) {
                        document.addEventListener('DOMContentLoaded', function() {
                            var lateTarget = document.head || document.documentElement;
                            if (lateTarget && style.parentNode !== lateTarget) {
