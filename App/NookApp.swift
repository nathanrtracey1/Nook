...
struct NookApp: App {
    ...
    private func setupApplicationLifecycle() {
        ...
        // Nook AdBlock Engine: attach browser manager for allowlist access.
        NookAdBlockManager.shared.attach(browserManager: browserManager)
    ...
    func makeNSView(context: Context) -> NSView {
        ...
        DispatchQueue.main.async {
            if let window = view.window {
                ...
                // IMPORTANT: Merge desired style masks into the existing styleMask
                // instead of overwriting it. Overwriting can clear .fullScreen
                // while the window is transitioning, which triggers AppKit asserts.
                var style = window.styleMask
                style.insert([.titled, .closable, .miniaturizable, .resizable, .fullSizeContentView])
