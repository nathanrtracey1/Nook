...
final class Profile: NSObject, Identifiable {
    ...
    /// True when this is the main default profile; used to opt into the system default website data store for native password autofill.
    var isDefault: Bool {
        let n = name.lowercased()
        return n == "default" || n == "default profile"
    }
    ...
    private static func createDataStore(for profileId: UUID) -> WKWebsiteDataStore {
        ...
        BrowserEnvironment.disableITP(on: store, label: "Profile.\(profileId.uuidString.prefix(8))")

        Task { @MainActor in
            await store.httpCookieStore.setCookiePolicy(.allow)
        }
