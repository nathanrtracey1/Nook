...
struct SpacesSideBarView: View {
    ...
    private var mainSidebarContent: some View {
        ...
        return VStack(spacing: 8) {
            ...
            // Favorites grid only; space-pinned tabs live under each space name (Arc-style)
        ...
        .background(
            GeometryReader { geo in
                Color.clear
                    ...
                    // Use onGeometryChange to avoid continuous layout invalidations
    ...
    private func spacesContent(spaces: [Space]) -> some View {
        ...
        .onAppear {
            // Align the current page with the active space, but avoid
            // repeatedly mutating shared state during view updates.
            if let currentId = windowState.currentSpaceId,
               let targetIndex = spaces.firstIndex(where: { $0.id == currentId }) {
                ...
                activeSpaceIndex = 0
                // Defer space activation to the next run loop to avoid
                // the "Publishing changes from within view updates" warning.
                DispatchQueue.main.async {
                    browserManager.setActiveSpace(first, in: windowState)
                }
            }
        }
    ...
    // Use a cancellable task to debounce hiding the downloads menu so we do not
    // accumulate multiple timers on the main queue when the hover state changes quickly.
    ...
    private func handleSpaceIndexChange(_ newIndex: Int, spaces: [Space]) {
        ...
        // Activate the space
        // Defer to the next run loop to avoid mutating observable
        // state during SwiftUI's view update pass.
        DispatchQueue.main.async {
            self.browserManager.setActiveSpace(space, in: self.windowState)
        }
    }
