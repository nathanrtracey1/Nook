...
/// a best-effort atomic save using a child ModelContext pattern.
    ...
    struct SnapshotTab: Codable {
        ...
        /// For pinned tabs: root URL to reload when favicon is clicked
    ...
    private func upsertTab(in ctx: ModelContext, _ t: SnapshotTab) throws {
        ...
        if let e = existing {
            ...
            e.rootURLString = t.rootURLString
        } else {
            let e = TabEntity(
                id: t.id,
...
class TabManager: ObservableObject {
    ...
    /// Default inactivity threshold before a background tab is eligible for suspension (in seconds).
    /// 20 minutes balances memory savings with quick tab restore.
    private let inactiveThreshold: TimeInterval = 20 * 60
    ...
    // MARK: - Inactive Tab Suspension

    private func startSuspendTimer() {
        suspendTimer?.invalidate()
        // Check for inactive tabs every 5 minutes on the main run loop.
        suspendTimer = Timer.scheduledTimer(withTimeInterval: 5 * 60, repeats: true) { [weak self] _ in
            Task { @MainActor in
    ...
    /// Unload background tabs that have been inactive for longer than `inactiveThreshold`.
    /// Pinned tabs, tabs with active media, and the current tab are never suspended.
    func suspendInactiveTabs() {
        let now = Date()
        let currentId = currentTab?.id

        // Work on a stable snapshot of tabs for the current profile.
        let candidates = allTabsForCurrentProfile()

        for tab in candidates {
            // Never suspend the current tab.
            if tab.id == currentId {
                continue
            }

            // Essentials (global pinned or space-pinned) stay warm.
            if tab.isPinned || tab.isSpacePinned {
                continue
            }

            // Skip tabs with active media so we do not interrupt background playback or PiP.
            if tab.hasPlayingAudio || tab.hasPlayingVideo || tab.hasPiPActive {
                continue
            }
            ...
            // Respect inactivity threshold.
            let elapsed = now.timeIntervalSince(tab.lastAccessed)
    ...
    func setActiveTab(_ tab: Tab) {
        ...
        #endif
        // Update inactivity tracking for suspension logic
    ...
    func unloadTab(_ tab: Tab) {
        ...
        // Arc-style: when closing (minimizing) a space-pinned tab and it was the current tab,
        // if there are no regular tabs and no other loaded space-pinned tab, deselect and show quiet default page.
        if tab.isSpacePinned, let spaceId = tab.spaceId {
            clearSelectionIfQuietAfterUnloadingSpacePinned(tab, in: spaceId)
        }
    }

    /// When a space-pinned tab was unloaded and it was current in some window(s), clear selection in those windows
    /// if the space has no regular tabs and no other loaded space-pinned tab (so the quiet default page can show).
    private func clearSelectionIfQuietAfterUnloadingSpacePinned(_ unloadedTab: Tab, in spaceId: UUID) {
        guard let bm = browserManager, let registry = bm.windowRegistry else { return }
        let regularTabs = tabsBySpace[spaceId] ?? []
        let pinnedTabs = spacePinnedTabs[spaceId] ?? []
        let hasAnotherSelectableTab = regularTabs.contains { !$0.isUnloaded }
            || pinnedTabs.contains { $0.id != unloadedTab.id && !$0.isUnloaded }
        if hasAnotherSelectableTab { return }

        let space = spaces.first { $0.id == spaceId }
        for (_, windowState) in registry.windows where windowState.currentTabId == unloadedTab.id {
            windowState.currentTabId = nil
            windowState.activeTabForSpace[spaceId] = nil
        }
        space?.activeTabId = nil
        if currentTab?.id == unloadedTab.id {
            currentTab = nil
        }
    }
    ...
    func handleDragOperation(_ operation: DragOperation) {
        ...
        case (.essentials, .spaceRegular(let spaceId)):
            // Essentials -> Regular (specific space): drag pinned tab out → unpin
    ...
    func pinTab(_ tab: Tab) {
        ...
        tab.folderId = nil
        tab.isPinned = true
        if tab.rootURL == nil { tab.rootURL = tab.url }
    ...
    /// Update a pinned tab's root URL only (persist, no reload). Use for "Edit Pinned URL" dialog.
    func updatePinnedTabRootURL(tab: Tab, newURL: URL) {
        tab.rootURL = newURL
        persistSnapshot()
    }

    /// Reload a pinned tab to its root URL (e.g. when favicon is clicked). Uses rootURL if set, else current url.
    func reloadPinnedTabToRoot(_ tab: Tab) {
        let target = tab.rootURL ?? tab.url
        tab.loadWebViewIfNeeded()
        tab.loadURL(target)
    }

    /// Update a pinned tab's current URL and reload (legacy; prefer updatePinnedTabRootURL + reloadPinnedTabToRoot).
    func updatePinnedTabURL(tab: Tab, newURL: URL) {
        tab.url = newURL
        tab.rootURL = newURL
        persistSnapshot()
        tab.loadWebViewIfNeeded()
    ...
    private func toRuntime(_ e: TabEntity) -> Tab {
        ...
        if let root = e.rootURLString, let rootURL = URL(string: root) {
            t.rootURL = rootURL
        }
    ...
    @MainActor private var persistDebounceTask: Task<Void, Never>?

    public nonisolated func persistSnapshot() {
        Task { @MainActor [weak self] in
            await self?.persistSnapshotDebounced()
        }
    }

    @MainActor
    private func persistSnapshotDebounced() async {
        persistDebounceTask?.cancel()
        persistDebounceTask = Task {
            // Coalesce rapid calls into a single write roughly every 500ms.
        ...
        for (pid, arr) in pinnedByProfile {
            ...
            for (i, t) in ordered.enumerated() {
                tabSnapshots.append(.init(
                    ...
                    canGoBack: t.canGoBack,
                    canGoForward: t.canGoForward,
                    rootURLString: t.rootURL?.absoluteString
                ))
            }
        }
