...
class NookSettingsService {
    ...
    private let contentBlockingEnabledKey = "settings.contentBlockingEnabled"
    private let contentBlockerAllowlistKey = "settings.contentBlockerAllowlist"
    private let contentBlockerEnabledListIDsKey = "settings.contentBlockerEnabledListIDs"
    private let contentBlockerAggressiveModeKey = "settings.contentBlockerAggressiveMode"
    private let contentBlockerCustomListsKey = "settings.contentBlockerCustomLists"
    ...
    // BEGIN CUSTOM MODIFICATION — remove borders when sidebar hidden (custom UI preference)
    private let removeBordersWhenSidebarHiddenKey = "settings.removeBordersWhenSidebarHidden"
    ...
    /// Controls whether Nook follows the system appearance or forces light/dark.
    ...
    /// Global toggle for the Nook-native content blocker (uAssets-powered).
    var contentBlockingEnabled: Bool {
        didSet {
            userDefaults.set(contentBlockingEnabled, forKey: contentBlockingEnabledKey)
            NotificationCenter.default.post(
                name: .contentBlockingEnabledChanged,
                object: nil,
                userInfo: ["enabled": contentBlockingEnabled]
            )
        }
    }

    /// Hostnames where the Nook content blocker should always be OFF.
    /// Entries are stored as lowercased hosts without scheme (e.g. "example.com").
    var contentBlockerAllowlist: [String] = [] {
        didSet {
            userDefaults.set(contentBlockerAllowlist, forKey: contentBlockerAllowlistKey)
            NotificationCenter.default.post(
                name: .contentBlockerAllowlistChanged,
                object: nil,
                userInfo: ["hosts": contentBlockerAllowlist]
            )
        }
    }

    /// IDs of external filter lists that are enabled. When empty, defaults are used.
    var contentBlockerEnabledListIDs: [String] = [] {
        didSet {
            userDefaults.set(contentBlockerEnabledListIDs, forKey: contentBlockerEnabledListIDsKey)
            NotificationCenter.default.post(
                name: .contentBlockerListsChanged,
                object: nil,
                userInfo: ["ids": contentBlockerEnabledListIDs]
            )
        }
    }

    /// When true, applies extra generic blocking rules for aggressive ad removal.
    var contentBlockerAggressiveMode: Bool = false {
        didSet {
            userDefaults.set(contentBlockerAggressiveMode, forKey: contentBlockerAggressiveModeKey)
            NotificationCenter.default.post(
                name: .contentBlockerAggressiveModeChanged,
                object: nil,
                userInfo: ["enabled": contentBlockerAggressiveMode]
            )
        }
    }
    ...
    // BEGIN CUSTOM MODIFICATION — remove borders when sidebar hidden (custom UI preference)
    /// When true, content uses full width (no side padding) when sidebar and AI panel are both hidden.
    var removeBordersWhenSidebarHidden: Bool {
        didSet {
            userDefaults.set(removeBordersWhenSidebarHidden, forKey: removeBordersWhenSidebarHiddenKey)
        }
    }
    // END CUSTOM MODIFICATION
    ...
    // MARK: - Content Blocker Custom Lists

    struct ContentBlockerCustomList: Codable, Identifiable, Equatable {
        var id: String
        var name: String
        var urlString: String
    }

    var contentBlockerCustomLists: [ContentBlockerCustomList] = [] {
        didSet {
            if let data = try? JSONEncoder().encode(contentBlockerCustomLists) {
                userDefaults.set(data, forKey: contentBlockerCustomListsKey)
            }
        }
    }
    ...
    init() {
        ...
        userDefaults.register(defaults: [
            ...
            blockXSTKey: false,
            contentBlockingEnabledKey: false,
            ...
            removeBordersWhenSidebarHiddenKey: true,
            // END CUSTOM MODIFICATION
        ...
        self.contentBlockingEnabled = userDefaults.bool(forKey: contentBlockingEnabledKey)
        if let storedIDs = userDefaults.array(forKey: contentBlockerEnabledListIDsKey) as? [String] {
            self.contentBlockerEnabledListIDs = storedIDs
        } else {
            self.contentBlockerEnabledListIDs = []
        }
        if let storedAllowlist = userDefaults.array(forKey: contentBlockerAllowlistKey) as? [String] {
            self.contentBlockerAllowlist = storedAllowlist
        } else {
            self.contentBlockerAllowlist = []
        }
        if let data = userDefaults.data(forKey: contentBlockerCustomListsKey),
           let decoded = try? JSONDecoder().decode([ContentBlockerCustomList].self, from: data) {
            self.contentBlockerCustomLists = decoded
        } else {
            self.contentBlockerCustomLists = []
        }
        self.contentBlockerAggressiveMode = userDefaults.bool(forKey: contentBlockerAggressiveModeKey)
        ...
        // BEGIN CUSTOM MODIFICATION
        self.removeBordersWhenSidebarHidden = userDefaults.bool(forKey: removeBordersWhenSidebarHiddenKey)
        // END CUSTOM MODIFICATION
...
extension Notification.Name {
    ...
    static let contentBlockingEnabledChanged = Notification.Name("contentBlockingEnabledChanged")
    static let contentBlockerAllowlistChanged = Notification.Name("contentBlockerAllowlistChanged")
    static let contentBlockerListsChanged = Notification.Name("contentBlockerListsChanged")
    static let contentBlockerAggressiveModeChanged = Notification.Name("contentBlockerAggressiveModeChanged")
}
...
private struct WebContentBorderlessKey: EnvironmentKey {
...
extension EnvironmentValues {
    ...
    /// When true, web content should have no rounded corners or border (e.g. when sidebar is hidden and "remove borders" is on).
    var webContentBorderless: Bool {
        get { self[WebContentBorderlessKey.self] }
        set { self[WebContentBorderlessKey.self] = newValue }
    }
}
